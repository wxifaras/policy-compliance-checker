...
public class AdminController : ControllerBase
{
    ...
    public async Task<IActionResult> GetPolicyLogs([FromQuery] string? userId = null)
    {
        try
        {
            var logs = await _cosmosDBService.GetLogsAsync<PolicyLog>(DocumentType.Policy.ToString(), userId);
            return Ok(logs);
        }
        catch (Exception ex)
        {
    ...
    public async Task<IActionResult> GetEngagementLogs([FromQuery] string? userId = null)
    {
        try
        {
            var logs = await _cosmosDBService.GetLogsAsync<EngagementLog>(DocumentType.Engagement.ToString(), userId);
            return Ok(logs);
        }
        catch (Exception ex)
        {
